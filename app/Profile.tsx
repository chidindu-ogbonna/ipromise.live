    ...
    <div className={styles.profile}>
      <div className={styles.profileHeader}>
        ...
        <div className={styles.profileInfo}>
          ...
          {cv.general.website ?
            <a className={styles.website} key="website">{cv.general.website}</a>
          : null}
        </div>
      </div>
      ...
      {cv.allCollections.map((collection: any, index: number) => {
        return (
          <section className={styles.profileSection} key={index}>
            ...
            <div className={collection.name === "Contact" ? styles.contacts : styles.experiences}>
              {collection.items.map((experience: any, index: number) => {
                ...
                if (collection.name === "Contact") {
                  return <ContactItem key={`contact-${index}`} experience={experience}/>
                }

                return (
                  <ProfileItem key={`profile-${index}`} experience={experience}/>
                )
              })}
            </div>
          </section>
